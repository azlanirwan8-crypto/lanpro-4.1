...
| 146 | **Dropdown MasterData tidak seragam: sebagian ikon+nama, sebagian teks polos** — permintaan pemilik proyek: semua dropdown bersumber MasterData tampil seperti JENIS KATEGORI (ikon + nama). Dua sebab berbeda. (a) 12 titik memakai `<select>` HTML asli, yang SECARA STRUKTURAL tidak bisa merender ikon di dalam `<option>` — harus dikonversi ke `StyledDropdown`. (b) ~10 titik sudah memakai StyledDropdown tetapi memetakan MasterData jadi `{id,label}` saja, membuang kolom `icon`/`color` yang sudah ada isinya di basis data. Mesinnya sendiri sudah siap: StyledDropdown merender `opt.icon` lewat RenderIcon, dan `useMasterOptionItems` (#140) sudah membawa ikon+warna. Data juga siap: 19 dari 20 tipe lengkap ikonnya; hanya 3 baris `jabatan` kosong | **F12** | 🟡  | Sedang |       Tidak        | `BELUM`           | §14    |

### 1.2 SUDAH SELESAI — 141 item
