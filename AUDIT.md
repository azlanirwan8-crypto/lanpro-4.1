...
| 143 | **Panel Master Data tidak pernah mengisi `code`** — `INSERT INTO MasterData` di `master-data.routes.ts:42` mendaftarkan id/type/label/color/icon/order/description/fieldType/dropdownOptions/role_type, tetapi TIDAK `code`. Setiap baris yang ditambahkan pengguna lewat panel karena itu selamanya tanpa kode sampai `npm run db:seed-master` dijalankan. Inilah akar dua baris yatim yang ditemukan di #141; penyemai kini menambalnya secara susulan, tetapi gejalanya akan terus muncul selama INSERT-nya belum diperbaiki | **F3**  | 🟡  | Rendah |       Tidak        | `BELUM`           | §14    |

### 1.2 SUDAH SELESAI — 135 item
