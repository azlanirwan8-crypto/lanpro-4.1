...
| 136 | **JSON kanvas bocor sebagai subjudul di daftar Dokumentasi** — flowchart menumpang tabel `documents`; `flowchart.service.ts:64` menyimpan payload `{nodes,edges}` ke kolom `description`, dan `WikiView.tsx:954` merender `description` sebagai subjudul manusia untuk SEMUA dokumen. Akibatnya baris flowchart menampilkan JSON mentah di Dokumentasi dan Editor Diagram Alur. Perlu keputusan: daftar dokumen menyaring `type === 'flowchart'`, atau payload kanvas dipindah ke kolomnya sendiri | **F12** | 🟡  | Sedang |       Tidak        | `BELUM`           | §12    |
