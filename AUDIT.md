...
**Sebaran per fase:** F1 0 · F2 0 · F3 0 · F5 0 · F6 0 · F7 0 · F8 0 · F9 0 · F10 0 · F11 1 · F12 3
...
| 144 | **Kategori flowchart tidak pernah tersimpan dan daftarnya beda sendiri** — dua cacat pada satu field. (1) `toFlowchartData` di `flowchart.service.ts:62` MENGERASKAN `category: "Panduan"`, dan `category` tidak pernah dikirim saat membuat/memperbarui; apa pun yang dipilih pengguna di modal dibuang diam-diam dan setiap diagram selalu tampil "Panduan" setelah dimuat ulang — keluarga cacat yang sama dengan #139. (2) Modal Diagram Alur mengeraskan 3 pilihan (PRD/Panduan/Laporan) sementara modal Dokumentasi membaca `jenis_dokumen` dari MasterData (9 pilihan); dua dropdown "Kategori Dokumen" pada tabel `Documents` yang sama menawarkan daftar berbeda | **F3**  | 🟡  | Sedang |       Tidak        | `BELUM`           | §14    |

### 1.2 SUDAH SELESAI — 137 item
