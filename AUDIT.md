...
**Sebaran per fase:** F1 0 · F2 0 · F3 0 · F5 0 · F6 0 · F7 0 · F8 0 · F9 0 · F10 0 · F11 1 · F12 5
...
| 139 | **Field Resolution tidak bisa disimpan sama sekali** — awalnya terlihat sebagai "MasterData bertipe `resolution` kosong"; tipe itu kini berisi 5 baris. Cacat sebenarnya lebih dalam: tabel `Tasks` TIDAK punya kolom `resolution`, dan rute update tugas hanya mengizinkan 15 field yang tidak memuatnya. UI melakukan pembaruan optimistis, jadi perubahannya tampak berhasil sampai halaman dimuat ulang. Hal yang sama berlaku untuk `release` dan `category` (tanpa kolom) serta `environment` dan `projectRisk` (kolom ada, tidak masuk allowlist)                                                                                                        | **F3**  | 🟡  | Sedang |       Tidak        | `BELUM`           | §14    |
| 140 | **Fase QA, status sprint, dan level task dikeraskan di 6 tempat** — SIT/UAT/PTR ditulis dua kali (AddSuiteModal dan QASuiteSidebar) sehingga menambah fase menuntut penyuntingan keduanya; ditambah status sprint, status kasus uji, dan Project Risk. DATANYA SUDAH DIBUAT: `sprint_status`, `qa_phase`, `qa_status`, `project_risk` kini ada di MasterData lewat `npm run db:seed-master`. Sisa pekerjaannya menyambungkan dropdown ke data itu. Level task (Epic/Standard/Subtask) dan kategori status (To Do/In Progress/Done) SENGAJA dibiarkan keras — keduanya metadata tentang struktur MasterData itu sendiri, menjadikannya MasterData akan melingkar | **F3**  | 🟢  | Sedang |       Tidak        | `BELUM`           | §14    |
| 141 | **`npm run db:seed-master` berhenti di tengah jalan** — blok "Migrasi Tasks.category (item #85)" menjalankan `UPDATE "Tasks" ... category = NULL`, padahal tabel `Tasks` tidak punya kolom `category`; skrip gagal dengan `column "category" does not exist`. Dua blok sesudahnya ("Melengkapi kode" dan "Dedup & migrasi rujukan") tidak pernah berjalan, sehingga 2 baris MasterData bertahan tanpa `code` (`priority/Urgent` dan `jenis_dokumen/'Contract '` — yang terakhir juga punya spasi di akhir label). Penyemaian tipe sendiri berhasil karena berjalan sebelum titik gagal                                                                          | **F3**  | 🟡  | Rendah |       Tidak        | `BELUM`           | §14    |

### 1.2 SUDAH SELESAI — 132 item
