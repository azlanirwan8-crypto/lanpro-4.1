...
| 145 | **14 komponen mati lagi, 2.453 baris** — ditemukan saat memverifikasi #137: `KpiMetricsRow` bukan satu-satunya. Tidak satu pun dirujuk dari kode mana pun (diperiksa atas SELURUH repo, bukan hanya `src/`): HeaderNetworkStatus, IssueTypeDropdown, SimpleModal, StatusSelect, DashboardCharts, MetricCard, SdlcBoard, SprintBanner, SprintPhaseAnalysis, TeamLeadMonitorCenter, TodayTaskSummary, KanbanAlertBanner, TimelineView, WikiEmptyState. Sebagian sempat saya sunting di #135 mengira dipakai — teksnya tidak pernah muncul di layar karena komponennya tidak pernah dirender. PERLU KEPUTUSAN: sebagian mungkin pekerjaan yang sengaja diparkir, jadi tidak dihapus sepihak. Catatan: 10 di antaranya terdaftar di `warna-baseline.json`, jadi penghapusan menuntut baseline warna diperbarui | **F7**  | 🟢  | Rendah |       Tidak        | `BELUM`           | §12    |

### 1.2 SUDAH SELESAI — 140 item
