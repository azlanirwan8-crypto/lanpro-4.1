...
| 154 | **Layar auth memakai varian layout split, bukan cover** — permintaan pemilik proyek setelah konsultasi desain: samakan tata letak dengan varian _cover_ Velzon (banner atas → gelombang → kartu mengambang → footer). Yang berubah HANYA pembungkus tata letak; logika form, validasi, SSO, dan modal lupa/reset kata sandi tidak disentuh. Latar banner tetap gradient + `VelzonFloatingParticles` yang sudah ada — TANPA foto, sebab foto eksternal menambah titik gagal offline dan membengkakkan LCP tanpa menyumbang kesan Velzon (yang menyumbang adalah gelombang + kartu mengambang + brand terpusat). Pembungkusnya kini ada di `AppContainer.tsx:3328–3370`, bukan di komponen auth, sehingga sekali diubah KEEMPAT layar auth ikut berubah: login, register, complete-registration, dan modal. Karena itu dikerjakan dua langkah: (1) ekstrak pembungkus ke `features/auth/components/AuthLayout.tsx` varian `split` dengan NOL perubahan visual, (2) baru tambah varian `cover`. Berkas baru wajib 100% token warna sejak baris pertama — `audit:warna` menolak berkas baru yang membawa warna keras; bila path SVG `#405189` di `AuthHeroPanel.tsx:46` ikut berpindah berkas, baselinenya harus diperbarui secara sadar. Gerbang: `lint:strict`, `test`, `audit:warna`, `audit:tema`, `build` hijau + verifikasi manual terang/gelap × mobile/tablet/desktop × 4 layar auth. | **F12** | ⚪  | Sedang |       Tidak        | `BELUM`           | §12    |

### 1.2 SUDAH SELESAI — 147 item
