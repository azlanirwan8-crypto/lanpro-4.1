...
| 139 | **Dropdown Resolution membaca tipe Master Data yang kosong** — `IssueTableRow` menyaring `type === "resolution"`, tetapi tabel MasterData tidak punya satu pun baris bertipe itu, jadi dropdown tampil kosong tanpa pesan apa pun                                                                                    | **F3**  | 🟡  | Rendah |       Tidak        | `BELUM`           | §14    |
| 140 | **Fase QA, status sprint, dan level task dikeraskan di 6 tempat** — SIT/UAT/PTR ditulis dua kali (AddSuiteModal dan QASuiteSidebar) sehingga menambah fase harus menyunting keduanya; ditambah status sprint, status kasus uji, Project Risk, level task, dan kategori status. Tidak satu pun punya tipe Master Data | **F3**  | 🟢  | Sedang |       Tidak        | `BELUM`           | §14    |
