...
| 152 | **Laporan PDF linimasa seluruhnya keras berbahasa Indonesia** — ditemukan saat menutup #149. `exportTimelineToPdf` (TimelinePanel.tsx:407–986) menyusun laporan eksekutif tiga halaman dengan ~238 teks lewat `doc.text()` jsPDF, termasuk judul, header tabel, narasi ringkasan, status yang dipetakan ulang ("Selesai", "Sedang Berjalan"), footer kerahasiaan, dan `toLocaleDateString("id-ID")`. Bentuk ini tidak pernah tersentuh pemindai mana pun — semuanya mencari teks di JSX, sedangkan ini argumen fungsi. Pengguna berbahasa Inggris mengekspor laporan berbahasa Indonesia. Diangkat terpisah dari #149 karena akarnya berbeda dan volumenya jauh lebih besar | **F12** | 🟡  | Sedang |       Tidak        | `BELUM`           | §12    |
| 153 | **ConfirmationModal di FlowchartContainer tak pernah bisa tampil** — ditemukan saat menutup #149. Satu-satunya yang menyetel `confirmModal.isOpen = true` adalah `deleteFlowchart` di `hooks/useFlowchartList.ts`, dan fungsi itu tidak pernah dipanggil: container memakai `deleteFlowchartApi` dengan teksnya sendiri. Jadi modal di FlowchartContainer.tsx:3895 beserta state confirmModal di hook adalah kode mati, termasuk judul dan pesannya yang keras berbahasa Indonesia. Tidak terdeteksi lint karena variabelnya didestrukturisasi dari objek                                                                                                                   | **F12** | 🟡  | Rendah |       Tidak        | `BELUM`           | §12    |

### 1.2 SUDAH SELESAI — 154 item
...
| **F7**  | **P0** | **Two-Tier RBAC** & validasi       | **#76**, #4, #81, #82, #83, #156        | 5–8  | **Tinggi**        | `JALAN` — Tahap 5b terbuka      | — **KOREKSI 23 Agu:** baris ini sempat berbunyi `SELESAI` dan "Seluruh item F7 SELESAI", padahal §19.8 masih menandai Tahap **5b** dan **6** sebagai `TERBUKA`. Tahap 6 ditutup lewat #156; **5b** (`can(action, module, projectId)` menggantikan 36 `hasPermission` di 13 berkas) MASIH terbuka |
