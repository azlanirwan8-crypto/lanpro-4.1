...
**Dihitung ulang 09 Sep 2026 (pasca #486 SELESAI):** F5 #305 · F6 #290 · F10 #433 #434 · F11 #30 · F13 #320. **#470–#486 → §1.2**. Rekap: **§24.9**. **#487 SELESAI kode 09 Sep 2026** (inline-add di bawah epic hormati pilihan type Story/Bug/Task).

**Keputusan pemilik 09 Sep 2026:** #482 opsi 1 — hanya system Administrator yang boleh ubah Reporter. **#483** — Hapus issue: Administrator sistem full akses; user non-admin mengikuti checklist `list.delete` (Issue Management). **#484** opsi A — hapus tombol Create Issue di toolbar Daftar Isu; sisakan quick-add bawah (+ modal lewat aksi mobile). **#485** — All Sprints dropdown (+ chip Active) di **pojok kanan** PageHeader Dashboard (`actions`), salam tetap kiri. **#486** — border inline-add Daftar Isu dibersihkan (hapus `border-y-2 border-primary`). Sisa temuan list: tipe vs badge Subtask · sembunyikan task key (belum nomor terpisah sampai disetujui).
