...
export const AuthNotificationProvider: React.FC<AuthNotificationProviderProps> = ({ children }) => {
  ...
  const handleAuthApiResponse = useCallback(
    (status: number, payload: any, onActivationClick?: () => void) => {
  ...
  return (
    ...
    >
      {children}
      <AuthToastContainer notifications={notifications} onDismiss={dismissNotification} />
    </AuthNotificationContext.Provider>
  );
};
...
export const VelzonSuccessIcon = () => (
  ...
  >
    ...
    <svg
      className="w-24 h-24 relative z-10 overflow-visible"
      viewBox="0 0 120 120"
      fill="none"
...
const VelzonSweetAlertModal: React.FC<VelzonSweetAlertModalProps> = ({
  notification,
  onDismiss,
}) => {
  ...
  return (
    ...
    >
      ...
      {/* Message Body */}
      <p className="text-sm text-content-muted font-normal leading-relaxed mb-6 px-2">{message}</p>
      ...
      <motion.button
        ...
        className="px-8 py-2.5 bg-primary hover:bg-primary-hover text-white rounded-md text-sm font-medium shadow-md transition-all cursor-pointer min-w-[110px]"
      >
        {displayBtnLabel}
      </motion.button>
    </motion.div>
