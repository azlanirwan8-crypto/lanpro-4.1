...
export const TeamManagementPanel = ({
  ...
  return (
    <div className="w-full h-[calc(100dvh-80px)] flex flex-col overflow-hidden text-left bg-surface-muted">
      <PageHeader
        breadcrumbs={[
                ...
                <ResponsiveTable className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-border-faint">
                    {filteredPeople.map((person: any, i) => {
                      ...
                      const initialsMatch = name.match(/\b\w/g);
                      const initials = (
                        initialsMatch ? initialsMatch.join("") : name.substring(0, 2)
                      )
                        .substring(0, 2)
                        .toUpperCase();
                    ...
                    <div className="mt-4 p-3 bg-surface-sunken rounded-lg border border-border-subtle/80 space-y-2">
                      ...
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-medium text-content-body">
                          {t("rakit.tasksCount", {
                            count: getUserTasks(selectedProfileUser).length,
                          })}
                        </span>
                      </div>
                    </div>
