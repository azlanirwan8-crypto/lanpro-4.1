    ...
    <div className="bg-surface flex flex-col font-sans text-left">
      ...
      <div className="px-5 py-4 border-b border-border-subtle/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-surface-sunken/40">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-indigo-500/10 border border-indigo-100 flex items-center justify-center text-primary shadow-2xs">
          ...
          <button
            ...
            className={cn(
              ...
              showAiAssistant
                ...
                : "bg-indigo-500/10 hover:bg-indigo-500/15 text-primary border-indigo-100"
            )}
          >
            {showAiAssistant ? (
              <>
                <ArrowLeft className="w-3.5 h-3.5" /> Tutup Asisten AI
              </>
            ) : (
              <>
        ...
        <div className="fixed inset-0 bg-overlay/40 backdrop-blur-xs flex items-center justify-center p-4 z-[9999] animate-in fade-in duration-200">
          <div className="bg-surface rounded-lg w-full max-w-2xl shadow-xl border border-border-subtle animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh] overflow-hidden">
            <div className="px-5 py-4 border-b border-border-faint flex items-center justify-between bg-surface-sunken/50 shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-md bg-indigo-500/10 flex items-center justify-center border border-indigo-100 text-primary">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-[11px] text-content-muted font-medium">
                    Ubah detail, catatan, atau penanggung jawab
                  </p>
                </div>
              </div>
              <button
      ...
      {showAiAssistant ? (
        ...
        <div className="space-y-4">
              ...
              <ResponsiveTable className="w-full border-collapse text-left text-xs">
                <thead>
                  <tr className="bg-primary-surface/5 border-b border-primary/15 text-xs sm:text-[11px] font-medium uppercase tracking-wider text-primary">
                    ...
                    <th className="py-3 px-4 min-w-[150px]">Context / Tags</th>
                      ...
                      <tr key={p.id} className="hover:bg-surface-sunken/70 transition-colors group">
                        ...
                        <td className="py-3 px-4 align-middle">
                          <div className="flex flex-wrap gap-1">
                            {p.fitur && (
                              <span className="px-2 py-0.5 bg-indigo-500/10 text-primary border border-indigo-100 rounded-md text-xs sm:text-[10px] font-medium">
                                {p.fitur}
                              </span>
                            )}
                            {p.system && (
                              <span className="px-2 py-0.5 bg-purple-500/10 text-purple-700 border border-purple-100/80 rounded-md text-xs sm:text-[10px] font-medium">
                                {p.system}
                              </span>
                            )}
                            {p.surrounding && (
                              <span className="px-2 py-0.5 bg-amber-500/10 text-amber-700 border border-amber-100/80 rounded-md text-xs sm:text-[10px] font-medium">
                                {p.surrounding}
                              </span>
                            )}
                            ...
                            return (
                              <button
                                ...
                                className={`inline-flex items-center justify-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium transition-all cursor-pointer border shadow-2xs active:scale-95 ${
                                  count > 0
                                    ? "bg-indigo-500/10 hover:bg-indigo-500/15 text-primary border-indigo-200/80"
                        ...
                        <td className="py-3 px-4 text-center align-middle">
                          ...
                          >
                            {isCompleted ? (
                              <span className="flex items-center justify-center gap-1 px-2.5 py-1 rounded-md text-xs sm:text-[10px] font-medium uppercase tracking-wider bg-emerald-500/10 text-emerald-700 border border-emerald-200/80 shadow-2xs hover:bg-emerald-500/15">
                                <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                                DONE
                              </span>
                            ) : (
                              <span className="flex items-center justify-center gap-1 px-2.5 py-1 rounded-md text-xs sm:text-[10px] font-medium uppercase tracking-wider bg-amber-500/10 text-amber-700 border border-amber-200/80 shadow-2xs hover:bg-amber-500/15">
                  ...
                  {canAdd && (
                    <tr className="bg-indigo-500/10 hover:bg-indigo-500/10 transition-colors">
                      ...
                      <td className="py-2.5 px-4 align-middle">
                        <input
                          type="text"
                          placeholder="Ketik concern baru..."
                          value={quickConcern}
                      ...
                      <td className="py-2.5 px-4 text-center align-middle">
                        <span className="px-2.5 py-1 rounded-md text-xs sm:text-[10px] font-medium uppercase tracking-wider bg-amber-500/10 text-amber-700 border border-amber-200/80">
                          PENDING
                        </span>
                      </td>
          ...
          <div className="px-6 py-3.5 bg-surface-sunken/60 border-t border-border-subtle flex items-center justify-between text-xs  text-content-secondary">
            <div>
              Menampilkan {paginatedPoints.length} dari {filteredPoints.length} poin (Total:{" "}
              {points.length})
            </div>
          ...
          <div className="relative bg-surface w-full max-w-lg h-full shadow-2xl border-l border-border-subtle flex flex-col animate-in slide-in-from-right duration-250 text-left">
            <div className="p-5 border-b border-border-faint flex items-center justify-between bg-surface-sunken/80 shrink-0">
              <div className="flex items-center gap-3 pr-4">
                <div className="w-10 h-10 rounded-xl bg-indigo-500/15 text-indigo-600 flex items-center justify-center border border-indigo-200 shrink-0">
            ...
            <div className="flex-1 p-5 overflow-y-auto flex flex-col gap-4 bg-surface-sunken/30">
              {threadComments.length === 0 ? (
                <div className="text-center py-16 px-4">
                  <div className="w-12 h-12 rounded-xl bg-indigo-500/10 border border-indigo-100 flex items-center justify-center mx-auto mb-3 text-indigo-400">
                  ...
                  <p className="text-xs sm:text-[11px] text-content-subtle  mt-1 leading-normal">
                    Jadilah yang pertama memberikan balasan atau instruksi tambahan untuk PIC topik
                    ini!
                  </p>
                </div>
              ) : (
