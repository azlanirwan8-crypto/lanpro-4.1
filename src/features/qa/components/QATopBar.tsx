...
export const QATopBar: React.FC<QATopBarProps> = ({
  ...
  return (
    <div className="page-title-box bg-surface border border-border-subtle/80 p-2.5 sm:p-4 rounded-lg shadow-xs">
      <PageHeader
        breadcrumbs={[{ label: t("nav.qa", "QA") }, { label: t("qaTop.title"), current: true }]}
        title={
          <span className="inline-flex items-center gap-2.5 min-w-0">
            <span className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-primary-surface text-content-inverse flex items-center justify-center font-medium shadow-xs shadow-primary/20 shrink-0">
              <FileSpreadsheet className="w-4 h-4 sm:w-5 sm:h-5" />
            </span>
            <span className="truncate">{t("qaTop.title")}</span>
          </span>
        }
        subtitle={<span className="hidden sm:inline">{t("qaTop.subtitle")}</span>}
        actions={
