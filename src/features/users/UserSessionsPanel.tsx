...
export const UserSessionsPanel: React.FC<UserSessionsPanelProps> = () => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-surface-muted overflow-y-auto">
      <PageHeader
        breadcrumbs={[
      ...
      <div className="px-3 md:px-5 pt-3 md:pt-4 pb-3 md:pb-5 space-y-3.5">
              ...
              <div className="shrink-0 w-[40%] max-w-[10.5rem] sm:w-auto sm:max-w-none">
                <StyledDropdown
                  ...
                  options={[
                    ...
                    {
                      id: "LOGGED_OUT",
                      label: t("sessionMonitor.statusLoggedOut", "Sudah Keluar"),
                    },
                    {
                      id: "FORCE_LOGOUT",
                ...
                {loadingActivities ? (
                  ...
                  <div className="relative border-l-2 border-border-subtle ml-2 pl-6 space-y-6">
                    {userActivities.map((act) => {
                      ...
                      const isCreate =
                        actionType.includes("CREATE") || actionType.includes("INSERT");
                      ...
                      const narrativeText = isDelete
                        ? t(
                            "sessionMonitor.actNarrativeDelete",
                            "Menghapus entitas / data pengguna"
                          )
                        : isCreate
                          ? t(
                              ...
                              <div className="mt-3 overflow-x-auto border border-border-subtle rounded-lg bg-surface-sunken">
                                <table className="w-full text-left text-xs">
                                  ...
                                  <tbody className="divide-y divide-border-subtle font-sans text-xs">
                                    {activeKeys.map((key) => {
                                      const oldVal = formatDiffVal(
                                        oldObj ? oldObj[key] : undefined
                                      );
                                      const newVal = formatDiffVal(
                                        newObj ? newObj[key] : undefined
                                      );
