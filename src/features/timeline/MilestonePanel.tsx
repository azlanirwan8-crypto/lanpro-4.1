...
import { Flag, Plus, Trash2, Pencil, Check, X, ChevronDown, ChevronUp } from "lucide-react";
...
export const MilestonePanel: React.FC<MilestonePanelProps> = ({ projectId, canWrite = true }) => {
  ...
  return (
    <section
      className="shrink-0 rounded-md border border-border-subtle/80 bg-surface px-3 py-2 md:px-5 md:py-3 shadow-2xs"
    ...
    >
      <button
        ...
        aria-controls="milestone-panel-body"
      >
        <div className="p-1.5 rounded-md bg-primary-surface/10 text-primary border border-primary/20 shrink-0">
          <Flag className="w-4 h-4" />
        </div>
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-medium text-content tracking-tight flex items-center gap-2">
            {t("roadmap.milestones")}
            {!loading && (
              <span className="text-[10px] font-normal text-content-muted tabular-nums">
                ({items.length})
              </span>
            )}
          </h3>
          <p
            className={cn(
              "text-[11px] md:text-xs text-content-muted line-clamp-1 md:line-clamp-none",
              !expanded && "hidden md:block"
            )}
          >
            {t("roadmap.milestonesHint")}
          </p>
        </div>
        <span
          className="md:hidden p-1.5 text-content-muted shrink-0 rounded-md hover:bg-surface-muted"
          aria-hidden
        >
      ...
      <div id="milestone-panel-body" className={cn("mt-2 md:mt-3", !expanded && "hidden md:block")}>
        {canWrite && (
          <form onSubmit={handleCreate} className="flex flex-col sm:flex-row gap-2 mb-2 md:mb-3">
            <input
              value={name}
            ...
            <input
              ...
              className="rounded-md border border-border-subtle bg-surface-muted px-3 py-2 text-sm text-content focus:outline-none focus:ring-1 focus:ring-primary sm:w-auto w-full"
            />
            <button
              type="submit"
              disabled={saving || !name.trim()}
              className="inline-flex items-center justify-center gap-1.5 rounded-md bg-primary text-content-inverse px-3 py-2 text-sm font-medium disabled:opacity-50 shrink-0"
            >
              <Plus className="w-4 h-4" />
              <span className="sm:inline">{t("roadmap.milestoneAdd")}</span>
            </button>
          </form>
        )}
        ...
        {loading ? (
          ...
          <ul className="space-y-2 max-h-36 md:max-h-48 overflow-y-auto">
            {items.map((m) => {
