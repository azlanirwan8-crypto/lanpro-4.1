...
export const NotebookLM: React.FC<NotebookLMProps> = ({
  project,
  userRole = "viewer",
  currentUser,
}) => {
    ...
    validFiles.forEach((file) => {
      ...
      reader.onload = (e) => {
        ...
        if (loadedCount === fileList.length) {
          toast.success(
            `${fileList.length} dokumen berhasil diunggah dan terpasang ke NotebookLM!`
          );
  ...
  const [selectedModel, setSelectedModel] = useState<
    "gemini-2.5-pro" | "gemini-3.6-flash" | "gemini-1.5-pro"
  >("gemini-2.5-pro");
  ...
  const [overviewType, setOverviewType] = useState<
    "summary" | "qa" | "podcast" | "briefing" | "study_guide"
  >("summary");
  ...
  const handleGenerateOverview = async (
    type: "summary" | "qa" | "podcast" | "briefing" | "study_guide"
  ) => {
    if (activeSources.length === 0) {
  ...
  return (
    <div
      id="notebooklm-container"
      className="flex flex-col min-h-[calc(100vh-4rem)] bg-surface-sunken p-4 md:p-6 text-content-strong font-sans space-y-4 animate-in fade-in duration-500"
    >
      {/* Panel 1: Top Actions */}
      <div className="bg-surface border border-border-subtle rounded-lg p-4 flex items-center justify-between shadow-soft shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center text-indigo-600 shadow-soft">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium text-content-secondary">Model AI:</span>
          <select
            value={selectedModel}
            onChange={(e: any) => setSelectedModel(e.target.value)}
            className="px-3 py-2 bg-surface border border-border-subtle/80 rounded-xl text-xs font-medium text-content-strong focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 shadow-soft transition-all cursor-pointer"
          >
      ...
      <div className="bg-surface border border-border-subtle rounded-lg shadow-soft flex-1 flex min-h-[600px] overflow-hidden">
        {/* Left Sidebar: Sources Panel */}
        <aside
          id="notebooklm-sources-sidebar"
          className="w-80 border-r border-border-subtle/60 bg-surface-sunken/40 flex flex-col shrink-0"
        >
          <div className="p-4 border-b border-border-subtle/60 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-indigo-600" />
              <span className="text-xs sm:text-[10.5px] font-medium uppercase tracking-wider text-content-body">
                Sumber Data ({sources.length})
              </span>
            </div>
            <button
              onClick={() => setShowAddModal(true)}
              className="p-1.5 bg-indigo-500/10 hover:bg-indigo-500/15 text-indigo-700 rounded-lg border border-indigo-500/30 transition-colors flex items-center gap-1 text-xs font-medium"
            ...
            {loadingSources ? (
              ...
              <div className="text-center py-12 px-4 border border-dashed border-border-subtle rounded-xl bg-surface-muted/50 ">
                ...
                <p className="text-xs sm:text-[11px] text-content-muted mt-1">
                  Klik tombol "+ Tambah" di atas atau buat Notulen Rapat / Wiki di proyek ini.
                </p>
              </div>
            ) : (
              <AnimatePresence>
                    ...
                    <motion.div
                      ...
                      className={`p-3 rounded-xl border transition-all cursor-pointer relative overflow-hidden ${
                        isSelected
                          ...
                          : source.active
                            ? "bg-surface border-border-subtle hover:border-purple-500/30 shadow-soft"
                  ...
                  return (
                    <button
                      ...
                      className={`relative px-3 py-1.5 rounded-xl text-xs font-medium whitespace-nowrap transition-all flex items-center gap-1.5 shrink-0 border ${
                        isSelected
                          ? "bg-purple-600 text-content-inverse border-purple-600 shadow-md shadow-purple-500/20"
                          : "bg-surface-muted text-content-secondary border-border-subtle /60 hover:bg-surface-strong "
                      }`}
                    >
                ...
                {(() => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="mt-3 p-3.5 bg-purple-500/10 border border-purple-500/30 rounded-xl flex items-start justify-between gap-4"
                    >
                      <div className="flex-1 min-w-0">
                      ...
                      <button
                        onClick={() =>
                          handleSendMessage(
                            `Jelaskan secara mendalam isi dari dokumen ${selectedDoc.title}`
                          )
                        }
                        className="px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-content-inverse text-xs font-medium rounded-lg shrink-0 shadow-soft transition-all flex items-center gap-1"
                      >
                        <Sparkles className="w-3 h-3" />
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                {chatHistory.length === 0 ? (
                  <div className="max-w-2xl mx-auto my-auto text-center py-12 px-6 bg-surface border border-border-subtle rounded-xl shadow-soft ">
                    <div className="w-12 h-12 bg-purple-500/15 rounded-xl border border-purple-500/30 flex items-center justify-center mx-auto mb-4">
                    ...
                    >
                      {msg.role === "assistant" && (
                        <div className="w-8 h-8 bg-purple-600 rounded-xl flex items-center justify-center shrink-0 shadow-md shadow-purple-500/20 border border-purple-400/30">
                          <Bot className="w-4 h-4 text-content-inverse" />
                        </div>
                      )}
                      ...
                      <div
                        className={`max-w-3xl rounded-xl p-4 border ${
                          msg.role === "user"
                            ? "bg-purple-600 border-purple-500 text-content-inverse rounded-tr-none"
                      ...
                      {msg.role === "user" && (
                        <div className="w-8 h-8 bg-surface-strong rounded-xl flex items-center justify-center shrink-0 border border-border-subtle ">
                ...
                {isGenerating && (
                  <div className="flex gap-4 items-center">
                    <div className="w-8 h-8 bg-purple-500/15 rounded-xl flex items-center justify-center shrink-0 border border-purple-500/30 animate-pulse">
              ...
              <div className="p-4 bg-surface/90 border-t border-border-subtle ">
                ...
                >
                  ...
                  <button
                    ...
                    className="px-5 py-3 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-content-inverse rounded-xl text-xs font-medium transition-all shadow-md shadow-purple-600/30 flex items-center gap-2 shrink-0"
                  >
                    <span>Kirim</span>
              ...
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-border-subtle ">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isPlayingAudio ? (
                    <button
                      onClick={stopAudio}
                      className="px-3 py-1.5 bg-red-600 hover:bg-red-500 text-content-inverse rounded-lg text-xs font-medium flex items-center gap-1.5 transition-colors"
                    >
                    ...
                    <button
                      onClick={() =>
                        handleGenerateTTS(overviewContent || "Pilih overview terlebih dahulu.")
                      }
                      disabled={!overviewContent || isAudioLoading}
                      className="px-3.5 py-1.5 bg-pink-600 hover:bg-pink-500 disabled:opacity-50 text-content-inverse rounded-lg text-xs font-medium flex items-center gap-1.5 shadow-md shadow-pink-600/20 transition-all"
                    >
              ...
              {isOverviewLoading ? (
                <div className="flex flex-col items-center justify-center py-20 text-content-muted text-xs gap-3">
                  ...
                  <p className="text-xs sm:text-[11px] text-content-muted">
                    Membaca dan merangkum {activeSources.length} dokumen terpasang.
                  </p>
                </div>
              ) : overviewContent ? (
                <div className="bg-surface border border-border-subtle rounded-xl p-6 shadow-soft relative">
                  <div className="flex items-center justify-between mb-4 pb-3 border-b border-border-subtle text-xs text-content-muted ">
                    ...
                    <button
                      ...
                      className="px-3 py-1 bg-purple-500/15 hover:bg-purple-200 text-purple-700 border border-purple-500/30 rounded-lg transition-colors flex items-center gap-1.5 text-xs font-medium"
                    >
                ...
                <div className="text-center py-20 bg-surface-muted/50 border border-dashed border-border-subtle rounded-xl">
                  ...
                  <p className="text-xs text-content-muted mb-6 max-w-md mx-auto">
                    Pilih salah satu tombol di bawah ini untuk membuat sintesis otomatis berbasis{" "}
                    {activeSources.length} dokumen.
                  </p>
            ...
            <div className="flex-1 flex flex-col p-6 overflow-y-auto">
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-border-subtle ">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-content-inverse rounded-md text-xs font-medium flex items-center gap-1.5 shadow-soft transition-all"
                >
                  <Plus className="w-3.5 h-3.5" />
              ...
              {studioNotes.length === 0 ? (
                <div className="text-center py-20 bg-surface-muted/50 border border-dashed border-border-subtle rounded-xl">
                  ...
                  <p className="text-xs text-content-muted max-w-md mx-auto">
                    Saat melakukan chat atau overview, klik tombol "Simpan ke Catatan" untuk
                    menyimpan poin penting di sini.
                  </p>
                </div>
              ) : (
        ...
        <div className="fixed inset-0 z-50 bg-overlay/40 backdrop-blur-sm flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex items-center justify-between pt-3 border-t border-border-subtle ">
              ...
              <button
                onClick={() => setShowAddModal(false)}
                className="px-4 py-2 bg-surface-muted hover:bg-surface-strong text-content-body rounded-xl text-xs font-medium transition-colors"
              >
                Tutup
              </button>
            </div>
          ...
          >
            <div className="flex items-center justify-between border-b border-border-subtle pb-3">
              ...
              <button
                ...
                className="text-content-subtle hover:text-content-secondary p-1 rounded-md"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-border-subtle ">
              <button
                onClick={() => setIsNewNoteModalOpen(false)}
                className="px-3.5 py-1.5 bg-surface-muted hover:bg-surface-strong text-content-body rounded-md text-xs font-medium"
              >
                Batal
              </button>
              <button
                ...
                className="px-4 py-1.5 bg-purple-600 hover:bg-purple-500 text-content-inverse rounded-md text-xs font-medium shadow-xs"
              >
                Simpan Catatan
              </button>
            </div>
