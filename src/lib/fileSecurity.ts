...
export function sanitizeFilename(originalName: string): string {
  ...
  // Extract ext and base name isomorphically
  ...
  // Generate random 12-char hex/alphanumeric suffix safely in browser & node
  let randomSuffix = "";
  if (
    ...
    randomSuffix =
      Math.random().toString(36).substring(2, 8) + Date.now().toString(36).substring(2, 8);
  }
...
export function validateFileBuffer(
  ...
  if (buffer.length >= 2) {
    ...
    if (magicHex.startsWith("4d5a")) {
      return {
        valid: false,
        error: "Gagal Mengunggah Dokumen: Terdeteksi file executable (.exe) yang dilarang!",
      };
    }
    // ELF (Linux binary)
    if (magicHex.startsWith("7f454c46")) {
      return {
        valid: false,
        error: "Gagal Mengunggah Dokumen: Terdeteksi file binary ELF yang dilarang!",
      };
    }
    // Java bytecode
    if (magicHex.startsWith("cafebabe")) {
      return {
        valid: false,
        error: "Gagal Mengunggah Dokumen: Terdeteksi Java bytecode yang dilarang!",
      };
    }
    // Shebang #!
    if (magicHex.startsWith("2321")) {
      return {
        valid: false,
        error: "Gagal Mengunggah Dokumen: Terdeteksi script executable yang dilarang!",
      };
    }
  }
  ...
  if (buffer.length >= 4) {
    ...
    if (ext === "png" && !headerHex.startsWith("89504e470d0a1a0a")) {
      // PNG header
    ...
    if (ext === "gif" && !headerHex.startsWith("47494638")) {
      // GIF8
      return { valid: false, error: "Gagal Mengunggah Dokumen: Header file GIF tidak valid!" };
    }

    if ((ext === "docx" || ext === "xlsx" || ext === "zip") && !headerHex.startsWith("504b0304")) {
      // PK zip header
      return {
        valid: false,
        error: "Gagal Mengunggah Dokumen: Header file dokumen/ZIP tidak valid!",
      };
    }
  ...
  if (
    first2048Str.includes("<script") ||
    first2048Str.includes("javascript:") ||
    first2048Str.includes("onerror=")
  ) {
    return {
      valid: false,
      error: "Gagal Mengunggah Dokumen: File mengandung script atau payload berbahaya!",
    };
  }
...
export function generatePresignedUrl(
  ...
  const token = crypto.createHmac("sha256", GET_SIGNING_SECRET()).update(dataToSign).digest("hex");
